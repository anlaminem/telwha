...
class CodingEngine:
    def __init__(self, workbook_path: Union[str, Path]):
        ...
        self.inputs_df: Optional[pd.DataFrame] = None
        self.mapping_df: Optional[pd.DataFrame] = None
        self.coding_df: Optional[pd.DataFrame] = None
    ...
    def _normalize_input_type(self, value: Any) -> str:
        if value is None or pd.isna(value):
            raise ValueError("input_type cannot be null")
        input_type = str(value).strip().lower()
        if not input_type:
            raise ValueError("input_type cannot be blank")
        return input_type

    def _normalize_coding_rule(self, value: Any) -> str:
        if value is None or pd.isna(value):
            raise ValueError("coding_rule cannot be null")
        coding_rule = str(value).strip()
        if not coding_rule:
            raise ValueError("coding_rule cannot be blank")
        return coding_rule
    ...
    def _build_input_types(self):
        ...
        if missing:
            raise ValueError(f"Inputs sheet must contain {sorted(required)}, missing: {sorted(missing)}")

        df["item_id"] = df["item_id"].apply(self._normalize_item_id)
        df["input_type"] = df["input_type"].apply(self._normalize_input_type)

        duplicate_ids = df.loc[df["item_id"].duplicated(keep=False), "item_id"].unique().tolist()
        if duplicate_ids:
            raise ValueError(f"Inputs sheet contains duplicate item_id values: {sorted(duplicate_ids)}")
    ...
    def _build_item_rules(self):
        ...
        missing = required - set(df.columns)
        if missing:
            raise ValueError(f"Coding sheet must contain {sorted(required)}, missing: {sorted(missing)}")

        df["item_id"] = df["item_id"].apply(self._normalize_item_id)
        df["coding_rule"] = df["coding_rule"].apply(self._normalize_coding_rule)

        duplicate_ids = df.loc[df["item_id"].duplicated(keep=False), "item_id"].unique().tolist()
        if duplicate_ids:
            raise ValueError(f"Coding sheet contains duplicate item_id values: {sorted(duplicate_ids)}")
    ...
    def _build_category_maps(self):
        ...
        missing = required - set(df.columns)
        if missing:
            raise ValueError(f"Mapping sheet must contain {sorted(required)}, missing: {sorted(missing)}")
        ...
        for _, row in df.iterrows():
            ...
            mapped_1_5 = self._normalize_int_like(row["normalized_score_1_5"], f"normalized_score_1_5 for {item_id}")

            if option_code is None:
                raise ValueError(f"Mapping option_code cannot be blank for item_id={item_id}")

            if mapped_1_5 not in DIRECT_MAP:
                raise ValueError(f"Invalid normalized_score_1_5 for {item_id}: {mapped_1_5}")

            self.category_maps.setdefault(item_id, {})

            if option_code in self.category_maps[item_id]:
                raise ValueError(f"Duplicate mapping for item_id={item_id}, option_code={option_code}")

            self.category_maps[item_id][option_code] = mapped_1_5
    ...
    def _validate_workbook_consistency(self):
        ...
        for item_id in sorted(input_ids):
            ...
            if coding_rule not in allowed_rules:
                raise ValueError(
                    f"Incompatible input_type/coding_rule for {item_id}: "
                    f"input_type={input_type}, coding_rule={coding_rule}"
                )
    ...
    def code_answer_record(self, answer_record: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if coding_rule == "mapped_cat_to_1_5_then_0_100":
            ...
            normalized_score_1_5 = item_map.get(normalized_option_code)
            ...
            if coding_rule == "direct_1_5_to_0_100":
                ...
                raise ValueError(f"Unsupported coding_rule for {item_id}: {coding_rule}")
    ...
    def code_answers_dict(self, answers: Dict[str, Dict[str, Any]]) -> pd.DataFrame:
        if not isinstance(answers, dict):
            raise TypeError("answers must be a dict keyed by item_id")
        ...
        for item_id, answer_record in answers.items():
            if not isinstance(answer_record, dict):
                raise TypeError(
                    f"answers[{item_id!r}] must be a dict like "
                    f"{{'item_id': ..., 'raw': ..., 'option_code': ...}}"
                )

            record = dict(answer_record)
            record.setdefault("item_id", item_id)
            coded = self.code_answer_record(record)

            if coded["item_id"] in seen_item_ids:
                raise ValueError(f"Duplicate item_id in answers: {coded['item_id']}")

            seen_item_ids.add(coded["item_id"])
            rows.append(coded)
    ...
    def code_answers_dataframe(
        ...
        if not isinstance(answers_df, pd.DataFrame):
            raise TypeError("answers_df must be a pandas DataFrame")
        ...
        if missing:
            raise ValueError(
                f"answers_df must contain columns {sorted(required_cols)}; missing: {sorted(missing)}"
            )
        ...
        for _, row in df.iterrows():
            ...
            if coded["item_id"] in seen_item_ids:
                raise ValueError(f"Duplicate item_id in answers_df: {coded['item_id']}")
